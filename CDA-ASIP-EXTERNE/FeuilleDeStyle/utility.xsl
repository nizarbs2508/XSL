							...
							<tr>
								...
								<td colspan="3">
									<xsl:variable name="count"
										select="count($name/n1:given[@qualifier != &apos;CL&apos; or not(@qualifier)])"/>
									<xsl:for-each
										select="$name/n1:given[@qualifier != &apos;CL&apos; or not(@qualifier)]">
										<xsl:choose>
											<xsl:when test="@qualifier = &apos;BR&apos;">
												<span style="font-weight:bold; ">
												<xsl:value-of select="."/>
	...
	<xsl:template name="show-guardian">
		...
		<xsl:for-each select="$guardian/n1:guardianPerson">
			<xsl:call-template name="show-name">
				...
				<xsl:with-param name="suffix" select="n1:name/n1:suffix"/>
				<xsl:with-param name="prefix" select="n1:name/n1:prefix"/>
				<xsl:with-param name="given" select="n1:name/n1:given"/>
				<xsl:with-param name="family" select="n1:name/n1:family"/>
			</xsl:call-template>
			<br/>
			<br/>
		...
		<xsl:if test="$asgnEntity/n1:assignedPerson/n1:name">
			<xsl:call-template name="show-name">
				...
				<xsl:with-param name="suffix"
					select="$asgnEntity/n1:assignedPerson/n1:name/n1:suffix"/>
				<xsl:with-param name="prefix"
					select="$asgnEntity/n1:assignedPerson/n1:name/n1:prefix"/>
				<xsl:with-param name="given" select="$asgnEntity/n1:assignedPerson/n1:name/n1:given"/>
				<xsl:with-param name="family"
					select="$asgnEntity/n1:assignedPerson/n1:name/n1:family"/>
			</xsl:call-template>
			<br/>
		</xsl:if>
	...
	<xsl:template name="show-relatedEntity">
		...
		<xsl:if test="$relatedEntity/n1:relatedPerson/n1:name">
			<xsl:call-template name="show-name">
				...
				<xsl:with-param name="suffix"
					select="$relatedEntity/n1:relatedPerson/n1:name/n1:suffix"/>
				<xsl:with-param name="prefix"
					select="$relatedEntity/n1:relatedPerson/n1:name/n1:prefix"/>
				<xsl:with-param name="given"
					select="$relatedEntity/n1:relatedPerson/n1:name/n1:given"/>
				<xsl:with-param name="family"
					select="$relatedEntity/n1:relatedPerson/n1:name/n1:family"/>
			</xsl:call-template>
		</xsl:if>
	</xsl:template>
		...
		<xsl:choose>
			<xsl:when test="$assoEntity/n1:associatedPerson">
				<xsl:for-each select="$assoEntity/n1:associatedPerson/n1:name">
					<xsl:call-template name="show-name">
						<xsl:with-param name="name" select="."/>
						<xsl:with-param name="suffix" select="./n1:suffix"/>
						<xsl:with-param name="prefix" select="./n1:prefix"/>
						<xsl:with-param name="given" select="./n1:given"/>
						<xsl:with-param name="family" select="./n1:family"/>
					</xsl:call-template>
					<br/>
				</xsl:for-each>
			</xsl:when>
